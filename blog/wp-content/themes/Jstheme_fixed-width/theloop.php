	...
	<?php /* Start the loop */ while ( have_posts() ) { the_post();	?>
		...
		<div id="post-<?php the_ID(); ?>" class="<?php k2_post_class($post_index++, in_category($k2asidescategory)); ?>">
			<div class="entry-head">
				<h3 class="entry-title"><span class="date" style="margin-right:10px;"><?php echo get_the_time('dM'); ?></span><a href="<?php the_permalink(); ?>" rel="bookmark" title='<?php printf( __('Permanent Link to "%s"','k2_domain'), wp_specialchars(strip_tags(the_title('', '', false)),1) ); ?>'><?php the_title(); ?></a></h3>
